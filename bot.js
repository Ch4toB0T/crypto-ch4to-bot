...
async function fetchStockPrice(symbol) {
  ...
  const data = await res.json();
  if (!data.lastPrice) throw new Error('Not found');
  return { symbol, price: parseFloat(data.lastPrice), change: parseFloat(data.priceChangePercent), high: parseFloat(data.highPrice), low: parseFloat(data.lowPrice), volume: parseFloat(data.quoteVolume) };
}

const STOCK_PAIRS = ['BTCUSDT','ETHUSDT','BNBUSDT','SOLUSDT','XRPUSDT'];
...
const mainMenu = new InlineKeyboard()
  ...
  .text('🐸 Memecoins', 'memecoins').text('📈 Top Assets', 'stocks').row()
...
const vipMenu = new InlineKeyboard()
  ...
  .text('💎 Legit Memecoins', 'legit_memes').text('📊 Asset Stats', 'stock_stats').row()
  .text('📊 Main Menu', 'menu');
...
bot.command('price', async (ctx) => { try { const symbol = ctx.match.trim().toUpperCase(); if (!symbol) return ctx.reply('Usage: /price BTC'); const res = await fetch('https://api.binance.com/api/v3/ticker/price?symbol='+symbol+'USDT'); const data = await res.json(); if (!data.price) return ctx.reply('Not found. Try /price BTC'); ctx.reply('💰 '+symbol+': $'+parseFloat(data.price).toLocaleString(), { reply_markup: new InlineKeyboard().text('⬅️ Menu', 'menu') }); } catch { ctx.reply('❌ Error. Try again.'); } });
bot.command('stats', async (ctx) => { try { const symbol = ctx.match.trim().toUpperCase(); if (!symbol) return ctx.reply('Usage: /stats BTC'); const res = await fetch('https://api.binance.com/api/v3/ticker/24hr?symbol='+symbol+'USDT'); const data = await res.json(); if (!data.lastPrice) return ctx.reply('Not found. Try /stats BTC'); ctx.reply('📊 '+symbol+':\n\nPrice: $'+parseFloat(data.lastPrice).toLocaleString()+'\n24h Change: '+parseFloat(data.priceChangePercent).toFixed(2)+'%\n24h High: $'+parseFloat(data.highPrice).toLocaleString()+'\n24h Low: $'+parseFloat(data.lowPrice).toLocaleString()+'\nVolume: $'+Number(parseFloat(data.quoteVolume).toFixed(0)).toLocaleString(), { reply_markup: new InlineKeyboard().text('⬅️ Menu', 'menu') }); } catch { ctx.reply('❌ Error. Try again.'); } });
...
bot.callbackQuery('stocks', async (ctx) => { await ctx.answerCallbackQuery(); try { const res = await fetch('https://api.binance.com/api/v3/ticker/24hr'); const data = await res.json(); const list = STOCK_PAIRS.map(sym => { const t = data.find(d=>d.symbol===sym); if (!t) return null; return t.symbol.replace('USDT','')+'\n   $'+parseFloat(t.lastPrice).toLocaleString()+' | '+parseFloat(t.priceChangePercent).toFixed(2)+'%'; }).filter(Boolean).map((s,i)=>(i+1)+'. '+s).join('\n\n'); ctx.reply('📈 Top Assets (Live):\n\n'+list+'\n\n⚠️ Not financial advice.', { reply_markup: new InlineKeyboard().text('📊 Full Stats (VIP)', 'stock_stats').row().text('⬅️ Menu', 'menu') }); } catch { ctx.reply('❌ Error fetching data. Try again.', { reply_markup: new InlineKeyboard().text('⬅️ Menu', 'menu') }); } });
bot.callbackQuery('stock_stats', async (ctx) => { await ctx.answerCallbackQuery(); if (!isVip(ctx)) return ctx.reply('⭐ VIP only.', { reply_markup: new InlineKeyboard().text('⭐ Get VIP', 'vip') }); try { const res = await fetch('https://api.binance.com/api/v3/ticker/24hr'); const data = await res.json(); const list = STOCK_PAIRS.map(sym => { const t = data.find(d=>d.symbol===sym); if (!t) return null; return t.symbol.replace('USDT','')+'\n   Price: $'+parseFloat(t.lastPrice).toLocaleString()+'\n   Change: '+parseFloat(t.priceChangePercent).toFixed(2)+'%\n   High: $'+parseFloat(t.highPrice).toLocaleString()+'\n   Low: $'+parseFloat(t.lowPrice).toLocaleString()+'\n   Volume: $'+Number(parseFloat(t.quoteVolume).toFixed(0)).toLocaleString(); }).filter(Boolean).map((s,i)=>(i+1)+'. '+s).join('\n\n'); ctx.reply('📊 Full Asset Stats:\n\n'+list+'\n\n⚠️ Not financial advice.', { reply_markup: vipMenu }); } catch { ctx.reply('❌ Error fetching data. Try again.', { reply_markup: vipMenu }); } });
