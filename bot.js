...
async function fetchMemecoins() {
  ...
  try {
    const res = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=dogecoin,shiba-inu,pepe,bonk,floki,brett,mog-coin,book-of-meme&order=market_cap_desc&per_page=8&page=1&price_change_percentage=24h');
...
const mainMenu = new InlineKeyboard()
  ...
  .text('🐸 Memecoins', 'memecoins').text('🧠 Learn', 'learn').row()
  .text('🛡️ Safe', 'safe').text('📖 Terms', 'terms').row()
  .text('🆘 Support', 'support').text('⭐ VIP Access', 'vip');
...
bot.callbackQuery('gainers', async (ctx) => { await ctx.answerCallbackQuery(); try { const res = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&per_page=10&page=1&price_change_percentage=24h'); const data = await res.json(); const list = data.sort((a,b)=>b.price_change_percentage_24h-a.price_change_percentage_24h).slice(0,5).map((c,i)=>(i+1)+'. '+c.name+' +'+c.price_change_percentage_24h.toFixed(2)+'%').join('\n'); ctx.reply('📈 Top Gainers:\n\n'+list, { reply_markup: new InlineKeyboard().text('⬅️ Menu', 'menu') }); } catch { ctx.reply('❌ Error fetching data. Try again.', { reply_markup: new InlineKeyboard().text('⬅️ Menu', 'menu') }); } });
bot.callbackQuery('losers', async (ctx) => { await ctx.answerCallbackQuery(); try { const res = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&per_page=10&page=1&price_change_percentage=24h'); const data = await res.json(); const list = data.sort((a,b)=>a.price_change_percentage_24h-b.price_change_percentage_24h).slice(0,5).map((c,i)=>(i+1)+'. '+c.name+' '+c.price_change_percentage_24h.toFixed(2)+'%').join('\n'); ctx.reply('📉 Top Losers:\n\n'+list, { reply_markup: new InlineKeyboard().text('⬅️ Menu', 'menu') }); } catch { ctx.reply('❌ Error fetching data. Try again.', { reply_markup: new InlineKeyboard().text('⬅️ Menu', 'menu') }); } });
bot.callbackQuery('volume', async (ctx) => { await ctx.answerCallbackQuery(); try { const res = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=volume_desc&per_page=5&page=1'); const data = await res.json(); const list = data.map((c,i)=>(i+1)+'. '+c.name+' $'+Number(c.total_volume).toLocaleString()).join('\n'); ctx.reply('💹 Highest Volume:\n\n'+list, { reply_markup: new InlineKeyboard().text('⬅️ Menu', 'menu') }); } catch { ctx.reply('❌ Error fetching data. Try again.', { reply_markup: new InlineKeyboard().text('⬅️ Menu', 'menu') }); } });
...
bot.callbackQuery('whales', async (ctx) => { await ctx.answerCallbackQuery(); if (!isVip(ctx)) return ctx.reply('⭐ VIP only.', { reply_markup: new InlineKeyboard().text('⭐ Get VIP', 'vip') }); try { const res = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=volume_desc&per_page=5&page=1'); const data = await res.json(); const list = data.map((c,i)=>(i+1)+'. '+c.name+' — $'+Number(c.total_volume).toLocaleString()+' volume').join('\n'); ctx.reply('🐋 Whale Activity:\n\n'+list, { reply_markup: vipMenu }); } catch { ctx.reply('❌ Error fetching data. Try again.', { reply_markup: vipMenu }); } });
...
bot.command('price', async (ctx) => { try { const symbol = ctx.match.trim().toLowerCase(); if (!symbol) return ctx.reply('Usage: /price bitcoin'); const res = await fetch('https://api.coingecko.com/api/v3/simple/price?ids='+symbol+'&vs_currencies=usd'); const data = await res.json(); if (!data[symbol]) return ctx.reply('Coin not found. Try /price bitcoin'); ctx.reply('💰 '+symbol.toUpperCase()+': $'+data[symbol].usd.toLocaleString(), { reply_markup: new InlineKeyboard().text('⬅️ Menu', 'menu') }); } catch { ctx.reply('❌ Error. Try again.'); } });
bot.command('stats', async (ctx) => { try { const symbol = ctx.match.trim().toLowerCase(); if (!symbol) return ctx.reply('Usage: /stats bitcoin'); const res = await fetch('https://api.coingecko.com/api/v3/coins/'+symbol); const data = await res.json(); if (!data.market_data) return ctx.reply('Coin not found. Try /stats bitcoin'); const m = data.market_data; ctx.reply('📊 '+data.name+':\n\nPrice: $'+m.current_price.usd.toLocaleString()+'\nMarket Cap: $'+m.market_cap.usd.toLocaleString()+'\n24h Change: '+m.price_change_percentage_24h.toFixed(2)+'%\nVolume: $'+m.total_volume.usd.toLocaleString(), { reply_markup: new InlineKeyboard().text('⬅️ Menu', 'menu') }); } catch { ctx.reply('❌ Error. Try again.'); } });
bot.start();
